...
The Naive Bayes model resulted in an overall 63.52% prediction accuracy. Some authors had high prediction accuracies than other. Aaron Pressman, for example, had a prediction accuracy of 94%. Whereas other authors such as Benjamin Kang Lim were harder to identify. Benjamin had a 26% prediction accuracy and was confused with Jane Macarteny 26% of the time. David Lawder also had a low prediction accuracy of 14% and was confused with Todd Nissen 50% of the time. Looking at David Lawder and Tom Nissen's documents, the association makes sense since both authors write about the auto industry.
...
library(nnet)
```
PCR using the first 500 principle components yields very similar results with a slightly higher prediction accuracy of 65.52%.
...
PCR with lasso yielded a lower prediction accuracy of 61.8%. Output not shown.
```
...
The following uses association rule mining on grocery purchases to identify interesting association rules for these shopping baskets. Support was set to > .01, meaning 1% of all transactions contain both x and y (~99). Confidence was set to > 0.5, meaning more than 50% of transactions that contain x also contain y. With these parameters and a length of <= 3, the strongest association is curd and yogurt with milk. There are several other associations with milk and a couple with other vegetables. It appears that milk and other vegetables are frequently purchased in certain conjunctions.
